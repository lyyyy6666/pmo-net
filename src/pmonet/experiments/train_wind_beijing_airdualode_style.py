...
from pmonet.data.beijing_airdualode_style import (  # noqa: E402
    AIRDUALODE_FEATURE_NAMES,
    Dataset_Beijing_AirDualODE_Style,
    custom_collate_fn_beijing_airdualode_style,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Beijing1718 AirDualODE-style PMO-Net training")
    parser.add_argument("--mode", type=str, default="smoke", choices=["train", "test", "smoke"])

    parser.add_argument("--root_path", type=str, default="./data/Beijing1718/airdualode_style")
    parser.add_argument("--station_path", type=str, default="station.csv")
    parser.add_argument("--stations_dir", type=str, default="stations")
    ...
    parser.add_argument("--freq", type=str, default="3h", choices=["3h"])
    ...
    parser.add_argument("--save_dir", type=str, default="./results/wind_beijing_airdualode_style")
    parser.add_argument("--checkpoints", type=str, default="./checkpoints/wind_beijing_airdualode_style")
...
def setup_logging():
    ...
    log_file = log_dir / f"train_final_wind_beijing_airdualode_style_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    logging.basicConfig(
        level=logging.INFO,
...
def validate_args(args) -> None:
    if args.hist_len != args.seq_len:
        raise ValueError(f"AirDualODE-style Beijing requires hist_len == seq_len. Got hist_len={args.hist_len}, seq_len={args.seq_len}")
    if args.features != "MS":
        raise ValueError("AirDualODE-style Beijing script requires features='MS'.")
    if args.target_idx != 0:
        raise ValueError(f"AirDualODE-style Beijing expects target_idx=0, got {args.target_idx}")
    if args.pred_len != 24:
        raise ValueError("AirDualODE-style Beijing expects pred_len=24.")
    if args.time_interval_hours != 3.0:
        raise ValueError(f"AirDualODE-style Beijing requires time_interval_hours=3.0, got {args.time_interval_hours}")
    ...
    if args.use_wind_advection:
        raise ValueError("AirDualODE-style Beijing entry currently keeps latent wind disabled because features are wind_speed/wind_direction, not wind_u/wind_v.")
    metadata = json.loads((Path(args.root_path) / args.metadata_path).read_text(encoding="utf-8"))
    if metadata.get("feature_names") != AIRDUALODE_FEATURE_NAMES:
        raise ValueError(f"Unexpected AirDualODE-style feature_names: {metadata.get('feature_names')}")


def prepare_dataset(flag: str, args) -> Dataset_Beijing_AirDualODE_Style:
    dataset = Dataset_Beijing_AirDualODE_Style(
        ...
        stations_dir=args.stations_dir,
    ...
    dataset.feature_metadata_path = str(Path(args.root_path) / args.metadata_path)
    dataset.wind_resolution = "airdualode_style_no_uv"
...
def prepare_loader(dataset, args, shuffle: bool):
    return DataLoader(
        ...
        drop_last=False,
        collate_fn=custom_collate_fn_beijing_airdualode_style,
    )


def log_split_summary(logger, dataset: Dataset_Beijing_AirDualODE_Style, split_name: str) -> None:
    split_def = dataset.split_ranges[dataset.flag]
    logger.info(
        "[%s] split indices: [%d, %d) out of total_steps=%d",
        split_name,
        split_def.start_idx,
        split_def.end_idx,
        dataset.total_time_steps,
    )
...
def log_dataset_debug(logger, dataset: Dataset_Beijing_AirDualODE_Style, split_name: str) -> None:
    logger.info("[%s] station csv protocol feature_names=%s", split_name, dataset.feature_names)
    logger.info("[%s] total tensor shape=(%d, %d, %d)", split_name, dataset.total_time_steps, dataset.num_nodes, dataset.num_features)
    ...
    logger.info("[%s] wind_enabled=%s wind_source=%s", split_name, dataset.wind_enabled, getattr(dataset, "wind_source", None))


def build_model(args, dataset: Dataset_Beijing_AirDualODE_Style, device: torch.device):
    sample = dataset[0]
...
def build_loss(args, dataset: Dataset_Beijing_AirDualODE_Style, device: torch.device):
    ...
    if args.use_observable_dyn_loss and args.lambda_observable_dyn > 0.0:
        observable_dynamics = ObservableDynamicsConsistency(
            ...
            use_geo_mask_for_wind=True,
            dt_hours=args.time_interval_hours,
            observable_dyn_mode="diffusion_decay",
            wind_graph_norm=args.wind_graph_norm,
        ).to(device)
...
def build_horizon_report(pred_real: np.ndarray, true_real: np.ndarray) -> dict[str, float]:
    ...
    for prefix, (start, end) in BEIJING_AIRDUALODE_SEGMENTS.items():
...
def log_horizon_report(logger, split_name: str, metrics: dict[str, float]) -> None:
    for prefix in BEIJING_AIRDUALODE_SEGMENTS:
        logger.info(
...
def log_single_batch_debug(logger, batch_info: dict[str, Any], output: torch.Tensor, dataset: Dataset_Beijing_AirDualODE_Style) -> None:
    ...
    logger.info("batch shape seq_y=%s", tuple(batch_info["seq_y"].shape))
...
def test_model(args, logger):
    ...
    test_loss, test_breakdown, preds_real, trues_real, preds_norm, trues_norm, metrics, _debug = collect_eval(
        model=model,
        loader=test_loader,
...
def run_smoke_case(case_name: str, case_args, logger, train_dataset, train_loader, device: torch.device) -> None:
    ...
    logger.info("output shape=%s loss total=%.6f pred=%.6f L_dyn=%.6f", tuple(output.shape), float(loss_dict["total"].detach()), float(loss_dict["pred"].detach()), float(loss_dict["L_dyn"].detach()))
    logger.info("meteo_future_raw excludes PM2.5=%s", train_dataset.target_idx not in train_dataset.meteo_cols)
    logger.info("wind_enabled=%s", train_dataset.wind_enabled)
...
def smoke_test(args, logger):
    ...
    cases = [
        ...
        ("no_wind_diffusion_decay", smoke_case_args(smoke_args, use_wind_advection=False, use_observable_dyn_loss=True, lambda_observable_dyn=1e-5, observable_dyn_mode="diffusion_decay")),
    ]
    for case_name, case_args in cases:
        run_smoke_case(case_name, case_args, logger, train_dataset, train_loader, device)
    logger.info("airdualode-style smoke test passed.")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("PMO-Net wind Beijing AirDualODE-style branch")
    logger.info("=" * 80)
